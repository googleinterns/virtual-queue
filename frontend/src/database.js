...
export const database_call = {
    ...
    //return userId of currently loggged in user
    getUserId: function(){
    ...
    isUserInQueue: function(storeId, userId, callBack){
        ...
        dbRef.child(this.getStorePath(userId)).orderByChild(STORE_ID_STRING).equalTo(storeId).once("value", snap =>{
            if(snap.exists()){
                callBack(true);
            } 
            else{
    ...
    getCurrentUserKey: function(storeId, userId, callBack){
        let dbRef = firebase.database().ref();
        dbRef.child(this.getUserPath(storeId)).orderByChild(USER_ID_STRING).equalTo(userId).once("value", snap => {
